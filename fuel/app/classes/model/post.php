  ...
  inner join sites site on p.site_id = site.id  inner join facilities f on p.facility_id = f.id inner join users_metadata um on p.contributor_id = um.parent_id inner join repairers rp on p.repairer_id = rp.id left join comments c on p.reject_id = c.id left join comments c2 on p.complete_id = c2.id where um.key = \'nickname\' and p.station_id = :station_id and p.status = :status and  p.contributor_id <> :contributor_id order by p.updated_at desc'
    ...
    public static function get_contribution_list($input,$order,$search_material,$to_array = false)
    {
        ...
        if (is_array($repairer_search) && $repairer_search[0] !== '')
        {
            ...
            {
                $where_repairer .= $val;
                $where_repairer .= count($repairer_search) - 1 !== $i ? ',' : '';
            }
            $where_repairer .= ") and";
        }
        ...
        if (is_array($routes_search) && $routes_search[0] !== '')
        {
            ...
            {
                $where_routes .= $val;
                $where_routes .= count($routes_search) - 1 !== $i ? ',' : '';
            }
            $where_routes .= ") and";
        }
        ...
        if (is_array($facility_search) && $facility_search[0] !== '')
        {
            ...
            foreach ($facility_search as $i => $val)
            {
                ...
                $where_facility .= "'" . $facility . "'";
                $where_facility .= count($facility_search) - 1 !== $i ? ',' : '';
            }
            $where_facility .= ") and";
        }
        ...
        if (is_array($status_search) && $status_search[0] !== '')
        {
            ...
            foreach ($status_search as $i => $val)
            {
                ...
                $where_status .= "'" . $status . "'";
                $where_status .= count($status_search) - 1 !== $i ? ',' : '';
            }
            $where_status .= ") and";
        }
        ...
        if (is_array($architecture_ward_search) && $architecture_ward_search[0] !== '')
        {
            ...
            {
                $where_architecture_ward .= $val;
                $where_architecture_ward .= count($architecture_ward_search) - 1 !== $i ? ',' : '';
            }
            $where_architecture_ward .= ") and";
        }
    ...
    public static function csv_export($order,$search_material,$to_array = false)
    {
        ...
        foreach ($posts as $post) {
            ...
            // $tmp['thumbnail_before1'] = $tmp['thumbnail_before1'] ? rtrim(\Uri::base(false), '/') . $tmp['thumbnail_before1'] . PHP_EOL : '';
            // $tmp['thumbnail_before2'] = $tmp['thumbnail_before2'] ? rtrim(\Uri::base(false), '/') . $tmp['thumbnail_before2'] . PHP_EOL : '';
            // $tmp['thumbnail_before3'] = $tmp['thumbnail_before3'] ? rtrim(\Uri::base(false), '/') . $tmp['thumbnail_before3'] . PHP_EOL : '';
            // $tmp['thumbnail_after1'] = $tmp['thumbnail_after1'] ? rtrim(\Uri::base(false), '/') . $tmp['thumbnail_after1'] . PHP_EOL : '';
            // $tmp['thumbnail_after2'] = $tmp['thumbnail_after2'] ? rtrim(\Uri::base(false), '/') . $tmp['thumbnail_after2'] . PHP_EOL : '';
            // $tmp['thumbnail_after3'] = $tmp['thumbnail_after3'] ? rtrim(\Uri::base(false), '/') . $tmp['thumbnail_after3'] : '';
            $tmp['thumbnail_before1'] = $tmp['thumbnail_before1'] ? Input::server('HTTP_HOST') . $tmp['thumbnail_before1'] . PHP_EOL : '';
            $tmp['thumbnail_before2'] = $tmp['thumbnail_before2'] ? Input::server('HTTP_HOST') . $tmp['thumbnail_before2'] . PHP_EOL : '';
            $tmp['thumbnail_before3'] = $tmp['thumbnail_before3'] ? Input::server('HTTP_HOST') . $tmp['thumbnail_before3'] . PHP_EOL : '';
            $tmp['thumbnail_after1'] = $tmp['thumbnail_after1'] ? Input::server('HTTP_HOST') . $tmp['thumbnail_after1'] . PHP_EOL : '';
            $tmp['thumbnail_after2'] = $tmp['thumbnail_after2'] ? Input::server('HTTP_HOST') . $tmp['thumbnail_after2'] . PHP_EOL : '';
            $tmp['thumbnail_after3'] = $tmp['thumbnail_after3'] ? Input::server('HTTP_HOST') . $tmp['thumbnail_after3'] : '';
